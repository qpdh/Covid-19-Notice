...
public class DailyTotal extends Activity {
    ...
    BarChart chart;
    String wantday = "20210430";

    Covid_daily data = new Covid_daily(wantday);
    ArrayList<InfectionBydaily> lable = new ArrayList<InfectionBydaily>();
    AsyncTask<Void, Void, ArrayList<InfectionBydaily>> thread = data.execute();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            for(int i=lable.size()-1; i>-1; i--){
                labels.add(lable.get(i).state_dt);
                values.add(Integer.parseInt(lable.get(i).decide_cnt));
